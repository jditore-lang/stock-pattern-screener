...
@st.cache_data(ttl=86400) # Caches the master list for 24 hours
def get_all_live_tickers():
    ...
    try:
        ...
        st.warning(f"FTP connection failed. Using fallback basket. Error: {e}")
...
def run_full_market_scan(all_tickers, pattern, market_cap_limit):
    ...
    # To run incredibly fast, we download the last 30 days of data in chunks of 50 tickers
    chunk_size = 50
    total_tickers = len(all_tickers[:500]) # For speed in this interface, we limit to top 500 liquid listings
    ...
    for i in range(0, total_tickers, chunk_size):
        ...
        try:
            # Download a 30-day block of historical candlesticks for the entire chunk at once
            ...
            for t in chunk:
                ...
                # Check for standard data anomalies
                ...
                # PATTERN MATH A: Bull Flag / Consolidation (Upward impulse + flat baseline)
                if pattern == "Bull Flag / Consolidation":
                    ...
                    if prior_return > 0.04 and recent_std < 0.012: # Up 4%+ then tight consolidation
                        matches.append({'Ticker': t, 'Price': close_prices[-1], 'History': close_prices[-7:].tolist()})
                        
                # PATTERN MATH B: High Volatility Breakout (Volume surge + price extension)
    ...
    # Build results and dynamically pull final metadata for matches only (Saves massive bandwidth)
    final_rows = []
    for m in matches:
        try:
...
with col1:
    ...
    pattern_rev = st.radio("Select Reversal:", ["None"]) # Expanded custom math arrays required later
with col2:
...
if selected_pattern != "None":
    ...
    # Run the comprehensive scanner engine across live data feeds
    ...
    if not screened_matches.empty:
        ...
        # Deep-Dive Section: Process 1-year math profiles for clicked selections
